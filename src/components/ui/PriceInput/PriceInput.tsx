...
export function PriceInput({ value, onChange, disabled, className, placeholder, style }: Props) {
  ...
  return (
    <input
      ...
      style={style}
      className={cn(
        "PriceInput h-9 w-[120] rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-raised)] px-3 text-right text-sm text-[var(--text-color-defult)] outline-none transition placeholder:text-[var(--text-color-ligth)] focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var(--color-accent-soft)] disabled:opacity-50",
        className
      )}
    />
  );
