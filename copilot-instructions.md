...
- `TrabajoDetailPage` is a client component that centralizes state for trabajo editing.
- `TrabajoForm` often accepts `externalFormAction`, `externalState`, and `externalIsPending` so parent components can control submit state.
- The `trabajo` edit screen combines `cobrado`, `prioridad`, `estado`, and the main form under one save flow.
...
- Key pages: `clientes`, `trabajos`, `precios`, `informacion-tecnica`, `repuestos`.
...
- When working with the `TrabajoDetailPage` and its save-state patterns.
